...
def image_transforms(mode='train', augment_parameters=[0.8, 1.2, 0.5, 2.0, 0.8, 1.2],
                     do_augmentation=True, transformations=None,  size=(256, 512+320)):
    if mode == 'train':
...
class ResizeImage(object):
    def __init__(self, train=True, size=(256, 512+320)):
        self.train = train
    ...
    def __call__(self, sample):
        if self.train:
            ...
            # left_image = np.reshape(left_image,[1,3,left_image.shape[0],left_image.shape[1]])
            # right_image = np.reshape(right_image,[1,3,right_image.shape[0],right_image.shape[1]])
            # # pad to (384, 1248)
            top_pad = 384-h
            left_pad = 1248-w
            left_image = np.lib.pad(left_image,((top_pad,0),(left_pad,0),(0,0)),mode='edge')
            right_image = np.lib.pad(right_image,((top_pad,0),(left_pad,0),(0,0)),mode='edge')
            	

            #left_image=  Image.fromarray(np.uint8(left_image))
     
            left_image = Image.fromarray(left_image.astype('uint8')).convert('RGB')
            right_image = Image.fromarray(right_image.astype('uint8')).convert('RGB')
...
class RandomFlip(object):
    ...
    def __call__(self, sample):
        ...
        if self.do_augmentation:
            if k > 0.5:
                left_image1 = self.transform(right_image)
                right_image1 = self.transform(left_image)
                sample = {'left_image': left_image1, 'right_image': right_image1}
        else:
            sample = {'left_image': left_image, 'right_image': right_image}
        return sample
